...
const FormatPage = (props) => {
  ...
  const getData = async () => {
    let [att, req, hols, days] = await Promise.all([
      ...
      role == "admin"
        ...
        : LeaveContext.getPending(currentUser.displayName),
    ...
    let bdayTrue = days[moment().format("DD-MM")]?.birthday?.includes(
      currentUser.displayName
    );
    let anniTrue = days[moment().format("DD-MM")]?.anniversary?.includes(
      currentUser.displayName
    );
    console.log();
    if (bdayTrue || anniTrue) {
      ...
      notes.push({
        name:
          (bdayTrue ? "Happy Birthday " : "") +
          (bdayTrue && anniTrue ? "and " : "") +
          (anniTrue ? "Congratulations on your Work Anniversary " : "") +
          `${currentUser.displayName}!`,
        icon: (
          <StarTwoTone twoToneColor="#fa8128" style={{ fontSize: "20px" }} />
        ),
      });
    ...
    if (moment().format("D") == "1") {
      ...
      notes.push({
        ...
        icon: (
          <AlertTwoTone twoToneColor="#fa8128" style={{ fontSize: "20px" }} />
        ),
      });
    }
    ...
    notes.push(
      ...
      att.length == 0
        ...
        : {
            ...
            // description: "Approval Required",
            badge: att.length,
            ref: ["/Attendance/regularize", { state: { active: "4" } }],
  ...
  if (
    role != "admin" &&
    ["expense", "employee", "attendance", "hr-leave"].includes(
