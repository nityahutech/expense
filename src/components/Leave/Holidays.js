import { Avatar, Space, Radio, Button, Calendar, Card, Col, Drawer, Row, Spin, Tooltip, Typography, Modal, Form, Input, DatePicker } from "antd"
...
import { capitalize, downloadFile, getUsers, showNotification } from '../../contexts/CreateContext';
...
const validateCSV = async (data, headers, model) => {
  ...
  const onFinish = (values) => {
    ...
    let matchingHolidayList = companyholiday.filter((item) => { return item.name == newHoliday.name })
...
const disabledDate = (current) => {
  ...
  console.log(totaldays, leavedays);
    ...
    />
        ...
        <Drawer
                        width={300}
                        title="List of Holiday" placement="right" onClose={() => setOpen(false)} open={open}>
